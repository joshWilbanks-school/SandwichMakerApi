...
def delete(db: Session, order_id):
    ...
    # make sure there is no order detail currently using this order first
    db_order_details = db.query(models.OrderDetail).filter(models.OrderDetail.order_id == order_id)

    if db_order_details is not None:
