...
def delete(db: Session, sandwich_id):
    ...
    # make sure there is no order detail using this sandwich first
    db_order_detail_exists = db.query(models.OrderDetail).filter(models.OrderDetail.sandwich_id == sandwich_id).first()

    if db_order_detail_exists is not None:
        return Response("There is an order detail that is currently using this order. Please remove the order detail first.", status.HTTP_406_NOT_ACCEPTABLE)
